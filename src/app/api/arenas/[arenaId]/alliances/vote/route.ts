...
/**
 * POST /api/arenas/[arenaId]/alliances/vote
 * Body: { allianceId: string; vote: "stay" | "betray" }
 */
export async function POST(
  ...
  const body = await request.json() as { allianceId?: string; vote?: string };
  if (!body.allianceId) return Response.json({ error: "allianceId required" }, { status: 400 });
  if (body.vote !== "stay" && body.vote !== "betray") {
    return Response.json({ error: "vote must be 'stay' or 'betray'" }, { status: 400 });
  }
  ...
  const res = await fetch(`${ENGINE_URL}/internal/alliances/vote`, {
    method: "POST",
    headers: { "x-internal-key": INTERNAL_KEY, "Content-Type": "application/json" },
    body: JSON.stringify({ allianceId: body.allianceId, participantId: participant.id, vote: body.vote }),
  });
