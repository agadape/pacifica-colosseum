...
// ── Wave fragment shader (Perlin fbm noise) ───────────────────────────────────
...
// ── Bayer 8×8 ordered-dither post effect ─────────────────────────────────────
...
class RetroEffectImpl extends Effect {
  ...
  get colorNum(): number   { return (this.uniforms.get("colorNum")!  as THREE.Uniform<number>).value; }
  set pixelSize(v: number) { (this.uniforms.get("pixelSize")! as THREE.Uniform<number>).value = v; }
  get pixelSize(): number  { return (this.uniforms.get("pixelSize")! as THREE.Uniform<number>).value; }
}
...
function DitheredWaves({
  ...
  waveSpeed: number; waveFrequency: number; waveAmplitude: number;
  waveColor: [number, number, number]; colorNum: number; pixelSize: number;
  disableAnimation: boolean; enableMouseInteraction: boolean; mouseRadius: number;
}) {
  ...
  useEffect(() => {
    ...
    const cur = waveUniformsRef.current.resolution.value;
    if (cur.x !== w || cur.y !== h) cur.set(w, h);
  }, [size, gl]);
  ...
  return (
    <>
      ...
      {/* Invisible pointer-capture mesh */}
