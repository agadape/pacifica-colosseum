  ...
  if (isLoading) {
    return (
      <div className="bg-surface rounded-2xl border border-border-light p-5">
        ...
        <div className="h-16 bg-bg-primary rounded-lg animate-pulse" />
      </div>
    );
  }
            ...
            <tbody>
              {positions.map((pos: Record<string, unknown>, i: number) => {
                ...
                return (
                  ...
                  >
                    <td className="py-2 font-mono font-semibold text-text-primary">{symbol}</td>
                    <td className={`py-2 font-semibold ${sideStr === "bid" ? "text-success" : "text-danger"}`}>
                      {sideStr === "bid" ? "Long" : "Short"}
                    </td>
                    <td className="py-2 text-right font-mono text-text-primary">{size.toFixed(4)}</td>
                    <td className="py-2 text-right font-mono text-text-secondary">{entryPrice.toFixed(2)}</td>
                    <td className="py-2 text-right font-mono text-text-primary">
                      {markPrice?.toFixed(2) ?? "—"}
                    </td>
                    <td className={`py-2 text-right font-mono font-semibold ${unrealizedPnl >= 0 ? "text-success" : "text-danger"}`}>
